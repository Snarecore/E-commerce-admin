...
const PolicyElevenForm = () => {
    const { postMutation, fetchData, handleApiMutation } = useAPI();
    const [fieldValues, setFieldValues] = useState(initialFieldValues);
    const apiUrl = apiConfig.setting.policy.policyElevenUrl;
    ...
    const handleSubmitForm = async () => {
        ...
        const result = await handleApiMutation({
            ...
            url,
            body: fieldValues,
            invalidateQueryKey: [privacyPolicyCmsQueryKey],
        ...
        if (result?.success) {
            fetchPrivacyPolicyCmsData();
        }
    };

    const [isLoading, setIsLoading] = useState<boolean>(true);
    const fetchPrivacyPolicyCmsData = async () => {
        setIsLoading(true);
        try {
...
export default PolicyElevenForm;
